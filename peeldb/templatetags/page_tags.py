...
from peeldb.models import (AppliedJobs, JobPost, User, City, Skill, State,
                           Industry, Company, Qualification, AssessmentData,
                           DEGREE_TYPES)
...
def get_s3_url(key):
    ...
    stored_url = s3.generate_url(600, 'GET', bucket=settings.AWS_STORAGE_BUCKET_NAME, key=key, force_http=True)
    return stored_url
...
def get_latest_walkins():
    ...
    if not latest_walkins:
        latest_walkins = JobPost.objects.filter(job_type='walk-in', status='Live').prefetch_related(
            'location', 'company').order_by('-walkin_to_date')[:10]
...
def get_latest_jobposts():
    ...
    if not latest_jobposts:
        latest_jobposts = JobPost.objects.filter(
            status='Live').exclude(job_type='walk-in')[:10]
...
def get_latest_recruiters():
    ...
    if not latest_recruiters:
        latest_recruiters = User.objects.filter(
            Q(user_type='RR') | Q(user_type='AR') | Q(user_type='AA') & Q(is_active=True)
        ).annotate(
            num_posts=Count('jobposts')
        ).prefetch_related('company').order_by('-num_posts')[:7]
...
def get_industries():
    ...
    if not all_industries:
        all_industries = Industry.objects.filter(
            status='Active').annotate(num_posts=Count('jobpost')).order_by('-num_posts')[:17]
...
def get_all_industries():
    all_industries = Industry.objects.annotate(
        num_posts=Count('jobpost')).filter(status='Active').order_by('-num_posts')
    return all_industries
...
def get_skills():
    ...
    if not all_skills:
        all_skills = Skill.objects.annotate(
            num_posts=Count('jobpost')).filter(status='Active').exclude(name='Fresher').order_by('-num_posts')
...
def get_refine_skills(skills):
    ...
    if not all_refine_skills:
        all_refine_skills = list(Skill.objects.filter(status='Active').annotate(
            num_posts=Count('jobpost')).order_by('-num_posts'))
...
def get_refine_locations(locations):
    ...
    if not all_refine_locations:
        all_refine_locations = list(City.objects.annotate(
            num_posts=Count('locations')).filter(status='Enabled').order_by('-num_posts'))
        cache.set("all_refine_locations", all_refine_locations, 10000)
    if locations:
        each_location = locations.annotate(num_posts=Count('locations')).order_by('num_posts')
...
def get_refine_states(states):
    ...
    if not all_refine_states:
        all_refine_states = list(State.objects.annotate(
            num_posts=Count('state__locations')).filter(status='Enabled').order_by('-num_posts'))
        cache.set("all_refine_states", all_refine_states, 10000)
    if states:
        each_location = states.annotate(num_posts=Count('state__locations')).order_by('num_posts')
        for each in each_location.iterator():
            try:
...
def get_refine_industries(industry):
    ...
    if not all_refine_industries:
        all_refine_industries = list(Industry.objects.annotate(
            num_posts=Count('jobpost')).filter(status='Active').order_by('-num_posts'))
        cache.set("all_refine_industries", all_refine_industries, 10000)
    if industry:
        each_industry = industry.annotate(num_posts=Count('jobpost')).order_by('num_posts')
...
def get_refine_educations(education):
    ...
    if not all_refine_educations:
        all_refine_educations = list(Qualification.objects.annotate(
            num_posts=Count('jobpost')).filter(status='Active').order_by('-num_posts'))
...
def get_locations():
    ...
    if not all_locations:
        all_locations = City.objects.annotate(
            num_posts=Count('locations')).filter(status='Enabled').order_by('-num_posts')
...
def get_full_time_jobs():
    ...
    if not all_jobs:
        all_jobs = JobPost.objects.filter(
            job_type='full-time', status='Live').order_by('-published_on')[:10]
...
def get_internships():
    ...
    if not internship_locations:
        internship_locations = City.objects.filter(
            locations__job_type='internship', status='Enabled'
        ).annotate(num_posts=Count('locations')).distinct()
...
def get_government_jobs():
    ...
    if not all_jobs:
        all_jobs = JobPost.objects.filter(
            job_type='governament', status='Live')[:17]
...
def get_companies():
    ...
    if not all_companies:
        all_companies = Company.objects.filter(is_active=True).annotate(num_posts=Count('jobpost')).order_by('-num_posts')[:50]
...
def get_qualifications():
    ...
    if not latest_qualifications:
        latest_qualifications = Qualification.objects.annotate(
            num_posts=Count('jobpost')).filter(status='Active').order_by('-num_posts')
...
def get_related_skills(search_skills):
    ...
    jobs = JobPost.objects.filter(skills__in=search_skills).prefetch_related(
        Prefetch("skills", queryset=Skill.objects.filter(skill_type=status, status='Active').exclude(id__in=exclude)))
    skills = []
    for job in jobs:
    ...
    if len(skills) < 20:
        ...
        if not latest:
            latest = Skill.objects.filter(status='Active').exclude(
                id__in=exclude).annotate(num_posts=Count('jobpost')).order_by('-num_posts')
...
def is_liked_question(user, question):
    likes = AssessmentData.objects.filter(question__id=question, user=user, comment='', like=True)
    return likes.exists()


@register.filter()
def is_disliked_question(user, question):
    likes = AssessmentData.objects.filter(question__id=question, user=user, comment='', dislike=True)
    return likes.exists()


@register.filter()
def is_liked_answer(user, answer):
    likes = AssessmentData.objects.filter(solution__id=answer, user=user, comment='', like=True)
    return likes.exists()


@register.filter()
def is_disliked_answer(user, answer):
    likes = AssessmentData.objects.filter(solution__id=answer, user=user, comment='', dislike=True)
    return likes.exists()
