...
from peeldb.models import Ticket, STATUS, TICKET_TYPES, PRIORITY_TYPES, Attachment, Comment
...
def index(request):
    ...
    if request.method == 'GET':
        if request.user.is_agency_recruiter or request.user.is_recruiter:
            ...
            data = {'tickets': tickets, 'priorities': PRIORITY_TYPES, 'ticket_types': TICKET_TYPES}
            ...
            data = {'message': 'Sorry, No Ticket Found', 'reason': '''The URL may be misspelled or the ticket
...
def new_ticket(request):
    ...
    """
    if request.method == 'GET':
        return render(request, 'recruiter/tickets/new_ticket.html', {'priorities': PRIORITY_TYPES,
...
def edit_ticket(request, ticket_id):
    ...
    if request.method == 'GET':
        if ticket:
            template_name = 'recruiter/tickets/edit_ticket.html' if request.user.is_agency_recruiter or\
                            request.user.is_recruiter else 'dashboard/tickets/edit_ticket.html'
            data = {'priorities': PRIORITY_TYPES, 'ticket_types': TICKET_TYPES, 'ticket': ticket}
        else:
            reason = '''The URL may be misspelled or the ticket
                        you're looking for is no longer available.'''
            template_name = 'recruiter/recruiter_404.html'
            data = {'message_type': '404',
                    'message': 'Sorry, No Ticket Found',
                    'reason': reason}
        return render(request, template_name, data, status=200 if ticket else 404)
    validate_ticket = TicketForm(
...
def delete_attachment(request, attachment_id):
    ...
    if attachments:
        ...
        if request.user.is_staff or request.user == attachment.uploaded_by:
            ...
            data = {
                'error': True, 'response': 'This Attachment cant be deleted'}
        return HttpResponse(json.dumps(data))
    else:
        ...
        if request.method == 'GET':
            if tickets:
                ...
                if request.user.is_staff or request.user == ticket.user:
                    ...
                    return render(request, template_name, {'priorities': PRIORITY_TYPES,
                                                           'ticket_types': TICKET_TYPES,
                                                           'ticket': tickets[0],
                                                           'status': STATUS,
    ...
    if tickets:
        ...
        if request.user.is_staff or request.user == ticket.user:
            if request.POST.get('ticket_status'):
                ...
                Memail(ticket.user.email, mfrom, subject, rendered, user_active)
                data = {
                    'error': False, 'response': 'Ticket status changed Successfully'}
            else:
        ...
        if request.user.is_staff or request.user == ticket.user:
            ...
            if validate_comment.is_valid():
                comment = Comment.objects.create(
                    comment=request.POST.get('comment'), ticket=ticket, commented_by=request.user)
                if request.FILES:
                    for key, value in request.FILES.items():
                        attachment = Attachment.objects.create(
                            attached_file=value, uploaded_by=request.user)
                ...
                if request.user.is_superuser:
                    ...
                    user_active = True if ticket.user.is_active else False
                    Memail(ticket.user.email, mfrom, subject, rendered, user_active)
                return HttpResponse(json.dumps({'error': False, 'response': 'Comment added Successfully'}))
            else:
                return HttpResponse(json.dumps({'error': True, 'response': validate_comment.errors}))
    reason = "The URL may be misspelled or the ticket you're looking for is no longer available."
    return render(request, 'recruiter/recruiter_404.html', {'message_type': '404',
                                                            'message': 'Sorry, No Ticket Found',
                                                            'reason': reason}, status=404)
...
def edit_comment(request):
    ...
    if comments:
        ...
        data = {
            'error': True, 'response_message': "This comment can't edit by the User"}
    return HttpResponse(json.dumps(data))
...
def admin_tickets_list(request):
    ...
    if request.user.is_staff:
        ...
        return render(request, 'dashboard/tickets/admin_ticket_list.html', {'tickets': tickets})
    message = 'Sorry, No Ticket Found'
    reason = "The URL may be misspelled or the ticket you're looking for is no longer available."
    return render(request, '404.html', {'message_type': '404',
                                        'message': message,
                                        'reason': reason}, status=404)
...
def admin_ticket_view(request, ticket_id):
    ...
    if request.user.is_staff:
        tickets = Ticket.objects.filter(id=ticket_id)
        if tickets:
            return render(request, 'dashboard/tickets/ticket_view.html', {'ticket': tickets[0],
