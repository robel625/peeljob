...
job_seeker_login_required = user_passes_test(lambda u: False if u.is_staff or u.is_recruiter else True, login_url='/')
...
rec_login_required = user_passes_test(lambda u: True if u.is_recruiter or u.is_agency_recruiter else False, login_url='/')


def recruiter_login_required(view_func):
    decorated_view_func = login_required(rec_login_required(view_func), login_url='/recruiter/login/')
    return decorated_view_func
...
def Memail(mto, mfrom, msubject, mbody, user_active):
    ...
    # send_mail('the contact form subject','this is message',"robelgulima1@gmail.com", ['robelgulima@gmail.com'])
...
nsprefixes = {
    ...
    'cp': ('http://schemas.openxmlformats.org/package/2006/metadata/core-pr' 'operties'),
...
def get_resume_data(file):
    ...
    if file_format == 'pdf':
        try:
            os.system("pdftotext '%s' '%s'" % (settings.BASE_DIR + '/resume/' + file_name, settings.BASE_DIR + '/resume/' + f_name + '.txt'))
            each = open(settings.BASE_DIR + '/resume/' + f_name + '.txt', 'r').readlines()
            text = []
...
def postonpeel_fb(job_post):
    ...
    job_name = str(job_post.title) + ', for Exp ' + str(job_post.min_year) + ' - ' + str(job_post.min_year)
...
def get_current_date_time():
    ...
    published_date = datetime.datetime.strptime(str(today), '%Y-%m-%d %H:%M:%S') + datetime.timedelta(seconds=120)
    return published_date


def get_next_month():
    nextmonth = datetime.date.today() + relativedelta(months=1)
    nextmonth = datetime.datetime.strptime(str(nextmonth), '%Y-%m-%d').strftime("%Y-%m-%d")
    return nextmonth
...
def get_asia_time():
    ...
    published_date = datetime.datetime.strptime(str(asia_time), '%Y-%m-%d %H:%M:%S') - datetime.timedelta(seconds=3600)
    return published_date
        ...
        for i, j in enumerate(location):
            ...
            while True:
                if City.objects.filter(slug__iexact=j, status="Enabled"):
                    ...
                    final_location.append(City.objects.filter(slug__iexact=j, status="Enabled")[0].name)
                    k = i
                    break
                i = i + 1
                if i < len(location):
...
def get_ordered_skill_degrees(text, skills, degrees):
    slugs = list(degrees.values_list('slug', flat=True)) + list(skills.values_list('slug', flat=True))
    order = {}
    final = []
    for word in slugs:
...
def get_aws_file_path(input_file, folder_path, company_name):
    ...
    s3_url = AWS().push_to_s3(file_obj=open(os.path.join(path, file_name), 'rb'),
...
def get_absolute_url(job):
    ...
    if str(job.job_type) == 'internship':
        ...
        qs = '/' + qs + '-' + \
            str(job.min_year) + '-to-' + str(job.max_year) + \
            '-years-' + str(job.id) + '/'
    return qs
...
def get_meta(name, data):
    ...
    meta = db.meta_data.find_one({'name': name})
    if meta:
        meta_title = Template(meta.get('meta_title')).render(Context({'current_page': page}))
        meta_description = Template(meta.get('meta_description')).render(Context({'current_page': page}))
...
def get_meta_data(name, data):
    ...
    page = data.get('page')
    final_location = ', '.join(data.get('final_location')) if data.get('final_location') else ''
    ...
    if not meta_title and meta:
        meta_title = Template(meta.get('meta_title')).render(Context({'city': final_location,
                                                                      'skill': final_skill,
                                                                      'current_page': page}))
    if not meta_description and meta:
        meta_description = Template(meta.get('meta_description')).render(Context({'city': final_location,
                                                                                  'skill': final_skill,
                                                                                  'current_page': page}))
    if not h1_tag and meta:
        h1_tag = Template(meta.get('h1_tag')).render(Context({'city': final_location,
                                                              'skill': final_skill,
                                                              'current_page': page}))
    return meta_title, meta_description, h1_tag
...
def save_codes_and_send_mail(user, request, passwd):
    ...
    rendered = temp.render({'activate_url': url, 'user_email': user.email, 'user_mobile': user.mobile, 'user': user,
                            'user_password': passwd, 'user_profile': user.profile_completion_percentage})
