...
# from mpcomp.views import (get_social_referer)
from mpcomp.views import (jobseeker_login_required, Memail, get_prev_after_pages_count, get_valid_skills_list,
                          mongoconnection, get_meta_data, get_valid_locations_list, get_social_referer,
                          get_resume_data, handle_uploaded_file, get_valid_qualifications, get_meta,
                          save_codes_and_send_mail, get_ordered_skill_degrees, get_404_meta, rand_string)

from peeldb.models import (JobPost, AppliedJobs, User, City, Industry,
                           Skill, Subscriber, VisitedJobs, State, TechnicalSkill,
                           Company, UserEmail, Qualification)
# from pjob.calendar_events import (create_google_calendar_event,
#                                   get_calendar_events_list,
#                                   delete_google_calendar_event, get_service)
from psite.forms import (SubscribeForm, UserEmailRegisterForm, UserPassChangeForm, AuthenticationForm, ForgotPassForm)
...
months = [{"Name": "Jan", "id": 1}, {"Name": "Feb", "id": 2},
          {"Name": "Mar", "id": 3}, {"Name": "Apr", "id": 4},
          {"Name": "May", "id": 5}, {"Name": "Jun", "id": 6},
          {"Name": "Jul", "id": 7}, {"Name": "Aug", "id": 8},
          {"Name": "Sep", "id": 9}, {"Name": "Oct", "id": 10},
          {"Name": "Nov", "id": 11}, {"Name": "Dec", "id": 12}]
...
def subscribers_creation_with_skills(email, skill, user):
    subscribers = Subscriber.objects.filter(
        email=email, user=None, skill=skill)
    if subscribers:
        for each in subscribers:
            if user:
                sub = Subscriber.objects.create(email=each.email, skill=each.skill, user=user)
                while True:
...
def jobs_applied(request):
    if request.user.is_authenticated and request.user.user_type == 'JS':
        ...
        applied_jobs = AppliedJobs.objects.filter(user=request.user).exclude(ip_address='', user_agent='')
        suggested_jobs = []
        if not applied_jobs:
            user_skills = Skill.objects.filter(id__in=request.user.skills.all().values('skill'))
            suggested_jobs = JobPost.objects.filter(Q(skills__in=user_skills) | Q(location__in=[request.user.current_city]))
            suggested_jobs = list(suggested_jobs.filter(status='Live'))
        suggested_jobs = suggested_jobs + list(JobPost.objects.filter(status='Live').order_by('-published_on')[:10])
        items_per_page = 15
        ...
        template = 'mobile/jobs/applied_jobs.html' if request.is_mobile else 'candidate/applied_jobs.html'
...
def job_detail(request, job_title_slug, job_id):
    if not job_id or bool(re.search(r"[A-Za-z]", job_id)):
        ...
        return render(request, template, {'message': 'Sorry, No Jobs Found', 'job_search': True,
                                          'reason': reason}, status=404)
    job = JobPost.objects.filter(
        id=job_id).select_related('company', 'user').prefetch_related(
            'location', 'skills', 'industry', 'functional_area', 'job_interview_location').first()
    if job:
        ...
        if job.status == 'Live':
            ...
            if job.major_skill and job.major_skill.status == 'Active':
                ...
                return HttpResponseRedirect(job.skills.filter(status='Active').first().get_job_url())
            return HttpResponseRedirect(reverse('jobs:index'))
        else:
            template = 'mobile/404.html' if request.is_mobile else '404.html'
            return render(request, template, {'message': 'Sorry, No Jobs Found', 'job_search': True,
                                              'reason': "The URL may be misspelled or the page you're looking for is no longer available."}, status=404)
        ...
        #     meta_description = Template(meta.get('meta_description')).render(Context({'job': job}))
        template = 'mobile/jobs/detail.html' if request.is_mobile else 'jobs/detail.html'
        data = {'job': job, 'show_pop_up': show_pop, 'meta_title': meta_title,
        ...
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': message, 'reason': reason, 'job_search': True}, status=404)


def recruiter_profile(request, recruiter_name, **kwargs):
    current_url = reverse('recruiter_profile', kwargs={'recruiter_name': recruiter_name})
    ...
    if re.match(r'^/jobs/recruiter/(?P<recruiter_name>[a-zA-Z0-9_-]+)/', request.get_full_path()):
        url = request.get_full_path().replace(
            'jobs/', '').replace('recruiter', 'recruiters')
        return redirect(url, permanent=True)

    job_list = JobPost.objects.filter(
        user__username__iexact=recruiter_name, status='Live'
    ).select_related('company', 'user').prefetch_related(
        'location', 'skills', 'industry'
    ).order_by('-published_on').distinct()
    ...
    if user:
        ...
        if meta:
            meta_title = Template(meta.get('meta_title')).render(Context({'current_page': page, 'user': user[0]}))
            meta_description = Template(meta.get('meta_description')).render(Context({'current_page': page, 'user': user[0]}))
            h1_tag = Template(meta.get('h1_tag')).render(Context({'current_page': page, 'user': user[0]}))
        template = 'mobile/jobs/recruiter_detail.html' if request.is_mobile else 'jobs/recruiter_profile.html'
        return render(request, template, {'user': user[0],
                                          'job_list': job_list,
                                          'aft_page': aft_page,
                                          'after_page': after_page,
                                          'prev_page': prev_page,
        ...
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': 'Sorry, Recruiter profile unavailable', 'data_empty': True,
                                          'reason': "Unfortunately, we are unable to locate the recruiter you are looking for"}, status=404)
...
def recruiters(request, **kwargs):
    ...
    if 'page' in request.GET:
        ...
        return redirect(url, permanent=True)
    recruiters_list = User.objects.filter(
        Q(user_type='RR') | Q(user_type='AR') | Q(
            user_type='AA') & Q(is_active=True, mobile_verified=True)).annotate(
                num_posts=Count('jobposts')).prefetch_related('company').order_by('-num_posts')
    ...
    prev_page, previous_page, aft_page, after_page = get_prev_after_pages_count(page, no_pages)
    recruiters_list = recruiters_list[(page - 1) * items_per_page:page * items_per_page]
    meta_title, meta_description, h1_tag = get_meta('recruiters_list', {'page': page})
    template = 'mobile/jobs/recruiters_list.html' if request.is_mobile else 'jobs/recruiters_list.html'
    return render(request, template, {'recruiters': recruiters_list,
                                      'aft_page': aft_page,
                                      'after_page': after_page,
                                      'prev_page': prev_page,
                                      'previous_page': previous_page,
...
def index(request, **kwargs):
    ...
    searched_locations = searched_skills = searched_industry = searched_edu = searched_states = ''
...
def job_locations(request, location, **kwargs):
    ...
    if request.POST.get('refine_search') == 'True':
        ...
        final_location = final_location + list(searched_states.values_list('name', flat=True))
    elif state:
    ...
    if job_list:
        ...
        field = get_social_referer(request)
        show_pop = True if field == 'fb' or field == 'tw' or field == 'ln' else False
        meta_title, meta_description, h1_tag = get_meta_data('location_jobs', {'locations': searched_locations,
                                                                               'final_location': set(final_location),
                                                                               'page': page,
                                                                               'state': bool(state)})
        data = {'job_list': jobs_list, 'aft_page': aft_page,
                'after_page': after_page, 'prev_page': prev_page,
                'previous_page': previous_page, 'current_page': page,
                'last_page': no_pages,
                'no_of_jobs': no_of_jobs,
                'current_url': current_url,
                'skill_jobs': True,
        ...
        if final_location:
            ...
            status = 200
            meta_title, meta_description = get_404_meta('location_404', {'city': search})
        else:
            search = [location]
            status = 404
            meta_title = meta_description = ''
        ...
        return render(request, template, {'message': 'Unfortunately, we are unable to locate the jobs you are looking for',
                                          'meta_title': meta_title, 'meta_description': meta_description, 'job_search': True,
                                          'reason': reason, 'searched_locations': search, 'data_empty': status != 200}, status=status)
...
def job_skills(request, skill, **kwargs):
    ...
    if job_list.count() > 0:
        ...
        if searched_edu and not searched_skills:
            ...
            if meta:
                meta_title = Template(meta.get('meta_title')).render(Context({'current_page': page, 'degree': final_edu}))
                meta_description = Template(meta.get('meta_description')).render(Context({'current_page': page, 'degree': final_edu}))
                h1_tag = Template(meta.get('h1_tag')).render(Context({'current_page': page, 'degree': final_edu}))
        elif searched_edu and searched_skills:
            meta = db.meta_data.find_one({'name': 'skill_education_jobs'})
            if meta:
                search = ', '.join(searched_text)
                meta_title = Template(meta.get('meta_title')).render(Context({'current_page': page, 'search': search}))
                meta_description = Template(meta.get('meta_description')).render(Context({'current_page': page, 'search': search}))
                h1_tag = Template(meta.get('h1_tag')).render(Context({'current_page': page, 'search': search}))
        elif searched_skills:
            meta_title, meta_description, h1_tag = get_meta_data('skill_jobs', {'skills': searched_skills, 'final_skill': final_skill, 'page': page})
        else:
            meta_title, meta_description, h1_tag = get_meta_data('skill_jobs', {'final_skill': [skill], 'page': page})
            searched_text = [skill]
        data = {'job_list': jobs_list,
                'current_url': current_url,
                'aft_page': aft_page,
                ...
                'searched_job_type': request.POST.get('job_type') or request.GET.get('job_type'),
                'meta_title': meta_title,
                'meta_description': meta_description,
                'h1_tag': h1_tag,
        ...
        return render(request, template, {'message': 'Unfortunately, we are unable to locate the jobs you are looking for',
                                          'meta_title': meta_title, 'meta_description': meta_description, 'job_search': True,
                                          'reason': reason, 'searched_skills': search, 'data_empty': status != 200}, status=status)
...
def job_industries(request, industry, **kwargs):
    ...
    if job_list:
        ...
        if meta:
            meta_title = Template(meta.get('meta_title')).render(Context({'current_page': page, 'industry': searched_industry[0].name}))
            meta_description = Template(meta.get('meta_description')).render(Context({'current_page': page, 'industry': searched_industry[0].name}))
            h1_tag = Template(meta.get('h1_tag')).render(Context({'current_page': page, 'industry': searched_industry[0].name}))
        data = {'job_list': jobs_list,
                'aft_page': aft_page,
                'after_page': after_page,
                'prev_page': prev_page,
        ...
        if searched_industry:
            ...
            meta_title, meta_description = get_404_meta('industry_404', {'industry': industry})
        else:
        ...
        return render(request, template, {'message': 'Unfortunately, we are unable to locate the jobs you are looking for',
                                          'meta_title': meta_title, 'meta_description': meta_description, 'job_search': True,
                                          'reason': reason, 'data_empty': False if searched_industry else True}, status=200 if searched_industry else 404)
...
def job_apply(request, job_id):
    if (request.user.is_active or request.GET.get('apply_now')) and request.user.user_type == 'JS':
            ...
            if not AppliedJobs.objects.filter(user=request.user, job_post=job_post):
                if request.user.resume or request.user.profile_completion_percentage >= 50:
                    ...
                    AppliedJobs.objects.create(
                        user=request.user, job_post=job_post, status='Pending',
                        ip_address=request.META['REMOTE_ADDR'], user_agent=request.META['HTTP_USER_AGENT'])
                    message = 'Your Application successfully sent for ' + \
                        str(job_post.title) + ' at ' + job_post.company_name
                    ...
                    data = {
                        'error': True, 'response': 'Please complete your profile to apply for this job', 'url': reverse('my:profile')}
                    return HttpResponse(json.dumps(data))
            else:
                data = {
                    'error': True, 'response': 'User already applied for this job'}
                return HttpResponse(json.dumps(data))
        data = {'error': True, 'response': 'Job you are searching Not found'}
        return HttpResponse(json.dumps(data))
    if request.user.user_type == 'RR':
        data = {
            'error': True, 'response': 'Recruiter Not allowed to Apply for Jobs'}
        return HttpResponse(json.dumps(data))
    if request.user.is_staff:
        data = {
            'error': True, 'response': 'Admin Not allowed to Apply For jobs'}
        return HttpResponse(json.dumps(data))
    data = {'error': True, 'response':
            'You need to verify your Email to apply For this Job'}
    return HttpResponse(json.dumps(data))
    ...
    if job_post:
        ...
        if request.method == 'POST':
            if str(request.POST['is_delete']) == 'True':
                subscribers.delete()
                data = {
                    'error': False, 'response': 'Please Update Your Profile To Apply For a job '}
            else:
                data = {
                    'error': True, 'response': 'Please Update Your Profile To Apply For a job '}
            return HttpResponse(json.dumps(data))
        return render(request, 'unsubscribe.html', {'email': email, 'subscribers': subscribers})
    else:
        message = 'Sorry, no jobs available'
        reason = "Unfortunately, we are unable to locate the job you are looking for"
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': message, 'reason': reason}, status=404)
...
def month_calendar(request, year, month):
    ...
    year = current_year
    month = next(
        (item for item in months if item['id'] == int(month)), None)
    calendar_events = []
    if request.user.is_authenticated:
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('month'):
            month = next(
                (item for item in months if item['id'] == int(request.POST.get('month'))), None)
    ...
    jobs_list = JobPost.objects.filter(status='Live')
    return render(request, "calendar/year_calendar.html", {"requested_month": request.POST.get('month') if request.POST.get('month') else None,
                                                           "months": months,
                                                           "year": year,
                                                           "month": month,
...
def jobposts_by_date(request, year, month, date, **kwargs):
    current_url = reverse('jobposts_by_date', kwargs={'year': year, 'month': month, 'date': date})
    if kwargs.get('page_num') == '1' or request.GET.get('page') == '1':
        return redirect(current_url, permanent=True)
    if 'page' in request.GET:
        url = current_url + request.GET.get('page') + '/'
    ...
    results = JobPost.objects.filter(
        status='Live', last_date=day).order_by('-published_on')
    ...
    for event in events:
        if event.get('start_date') and event.get('end_date'):
            if str(day) >= str(event['start_date']) and str(day) <= str(event['end_date']):
                event_titles.append(event['summary'])
    events = JobPost.objects.filter(title__in=event_titles)
    if not results:
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': 'Sorry, no jobs available', 'job_search': True, 'data_empty': True,
                                          'reason': "Unfortunately, we are unable to locate the job you are looking for"}, status=404)
    ...
    if meta:
        meta_title = Template(meta.get('meta_title')).render(Context({'date': date, 'searched_month': day.strftime("%B"), 'year': year}))
        meta_description = Template(meta.get('meta_description')).render(Context({'date': date, 'searched_month': day.strftime("%B"), 'year': year}))
        h1_tag = Template(meta.get('h1_tag')).render(Context({'date': date, 'month': day.strftime("%B"), 'year': year}))
    return render(request, 'calendar/calendar_day_results.html', {'no_of_jobs': len(results),
...
def job_add_event(request):
    ...
    if not is_connected:
        ...
        if created == 'redirect':
            ...
            return redirect(jobpost.get_absolute_url() + '?event=success', permanent=False)
        else:
            return redirect(jobpost.get_absolute_url() + '?event=error', permanent=False)
...
def calendar_add_event(request):
    ...
    start_date = datetime.strptime(
        str(request.POST.get('start_date')), "%m/%d/%Y").strftime("%Y-%m-%d")
    last_date = datetime.strptime(
        str(request.POST.get('to_date')), "%m/%d/%Y").strftime("%Y-%m-%d")
...
def jobs_by_location(request, job_type):
    ...
    ).filter(num_locations__gte=1, status='Enabled').prefetch_related(
        Prefetch("state", queryset=City.objects.filter(
            status="Enabled", parent_city=None
    ...
    if meta:
        meta_title = Template(meta.get('meta_title')).render(Context({'job_type': job_type}))
        meta_description = Template(meta.get('meta_description')).render(Context({'job_type': job_type}))
        h1_tag = Template(meta.get('h1_tag')).render(Context({'job_type': job_type}))
    data = {'states': states, 'job_type': job_type, 'all_degrees': all_degrees[:10],
            'meta_title': meta_title, 'meta_description': meta_description, 'h1_tag': h1_tag}
    template = 'mobile/jobs/jobs_by_location.html' if request.is_mobile else 'jobs/jobs_by_location.html'
    return render(request, template, data)
...
def jobs_by_skill(request):
    ...
    if request.method == 'POST':
        if str(request.POST.get('alphabet_value')) != 'all':
            all_skills = all_skills.filter(name__istartswith=request.POST.get('alphabet_value'))
        if request.POST.get('sorting_value') and (str(request.POST.get('sorting_value')) == 'descending'):
            all_skills = all_skills.order_by('-name')
        else:
            all_skills = all_skills.order_by('name')
    meta_title, meta_description, h1_tag = get_meta('jobs_by_skills', {'page': 1})
    data = {'all_skills': all_skills, 'meta_title': meta_title,
            'meta_description': meta_description, 'h1_tag': h1_tag}
    template = 'mobile/jobs/jobs_by_skill.html' if request.is_mobile else 'jobs/jobs_by_skills.html'
    return render(request, template, data)
...
def fresher_jobs_by_skills(request, job_type):
    ...
    if meta:
        ...
        meta_description = Template(meta.get('meta_description')).render(Context({'job_type': job_type}))
        h1_tag = Template(meta.get('h1_tag')).render(Context({'job_type': job_type}))
    data = {'all_skills': all_skills, 'job_type': job_type, 'h1_tag': h1_tag,
            'meta_title': meta_title, 'meta_description': meta_description}
    template = 'mobile/jobs/fresher_jobs_by_skills.html' if request.is_mobile else 'jobs/fresher_jobs_by_skills.html'
    return render(request, template, data)


def jobs_by_industry(request):
    all_industries = Industry.objects.filter(status='Active').annotate(num_posts=Count('jobpost')).order_by('-num_posts')
    if request.method == 'POST':
        all_industries = all_industries.filter(name__icontains=request.POST.get('industry'))
        if request.POST.get('sorting_value') and (str(request.POST.get('sorting_value')) == 'descending'):
            all_industries = all_industries.order_by('-name')
        else:
            all_industries = all_industries.order_by('name')
    meta_title, meta_description, h1_tag = get_meta('jobs_by_industry', {'page': 1})
    data = {'all_industries': all_industries, 'h1_tag': h1_tag,
            'meta_title': meta_title, 'meta_description': meta_description}
    template = 'mobile/jobs/jobs_by_industries.html' if request.is_mobile else 'jobs/jobs_by_industries.html'
    return render(request, template, data)
...
def jobs_by_degree(request):
    ...
    if request.method == 'POST':
        if str(request.POST.get('alphabet_value')) != 'all':
            all_degrees = all_degrees.filter(name__istartswith=request.POST.get('alphabet_value'))
        if request.POST.get('sorting_value') and (str(request.POST.get('sorting_value')) == 'descending'):
            all_degrees = all_degrees.order_by('-name')
        else:
            all_degrees = all_degrees.order_by('name')
    meta_title, meta_description, h1_tag = get_meta('jobs_by_degree', {'page': 1})
    data = {'all_degrees': all_degrees, 'h1_tag': h1_tag,
            'meta_title': meta_title, 'meta_description': meta_description}
    template = 'mobile/jobs/jobs_by_degree.html' if request.is_mobile else 'jobs/jobs_by_degree.html'
    return render(request, template, data)
...
def full_time_jobs(request, **kwargs):
    ...
    searched_locations = searched_industry = searched_skills = searched_edu = searched_states = ''
    if request.POST.get('refine_search') == 'True':
        jobs_list, searched_skills, searched_locations, searched_industry, searched_edu, searched_states = refined_search(request.POST)
    else:
        search_dict = QueryDict('', mutable=True)
        search_dict.setlist('job_type', ['full-time'])
        jobs_list, searched_skills, searched_locations, searched_industry, searched_edu, searched_states = refined_search(search_dict)
...
def internship_jobs(request, **kwargs):
    request.session['formdata'] = ''
    jobs_list = JobPost.objects.filter(
        status='Live', job_type='internship'
    ).select_related('company').prefetch_related('location', 'skills')[:9]
    no_of_jobs = jobs_list.count()
    no_pages = int(math.ceil(float(no_of_jobs) / 20))
    page = get_page_number(request, kwargs, no_pages)
    if not page:
    ...
    if request.is_mobile:
        return render(request, 'mobile/jobs/list.html', {'job_list': jobs_list,
                                                         'aft_page': aft_page,
                                                         'after_page': after_page,
                                                         'prev_page': prev_page,
                                                         'previous_page': previous_page,
                                                         ...
                                                         'meta_description': meta_description,
                                                         'h1_tag': h1_tag})
    return render(request, 'internship.html', {'jobs_list': jobs_list[:10],
...
def city_internship_jobs(request, location, **kwargs):
    ...
    field = get_social_referer(request)
    show_pop = True if field == 'fb' or field == 'tw' or field == 'ln' else False
    meta_title, meta_description, h1_tag = get_meta_data('location_internship_jobs', {'searched_locations': [location],
...
def walkin_jobs(request, **kwargs):
    ...
    ).select_related('company', 'user').prefetch_related('location', 'skills', 'industry')
...
def each_company_jobs(request, company_name, **kwargs):
    ...
    if not company:
        ...
        company = company[0]
        items_per_page = 10
        job_list = company.get_jobposts().select_related(
            'company', 'user').prefetch_related('location', 'skills', 'industry').order_by('-published_on')
        no_of_jobs = job_list.count()
        no_pages = int(math.ceil(float(no_of_jobs) / items_per_page))
        page = get_page_number(request, kwargs, no_pages)
        if not page:
        ...
        if meta:
            meta_title = Template(meta[0]['meta_title']).render(Context({'current_page': page, 'company': company}))
            meta_description = Template(meta[0]['meta_description']).render(Context({'current_page': page, 'company': company}))
            h1_tag = Template(meta[0]['h1_tag']).render(Context({'current_page': page, 'company': company}))
        data = {'job_list': jobs_list,
                'aft_page': aft_page,
                'after_page': after_page,
                'prev_page': prev_page,
                ...
                'meta_description': meta_description,
                'h1_tag': h1_tag}
        template = 'mobile/jobs/company_jobs.html' if request.is_mobile else 'jobs/company_jobs.html'
        return render(request, template, data)
...
def companies(request, **kwargs):
    ...
    if 'page' in request.GET:
        ...
        return redirect(url, permanent=True)
    companies = Company.objects.annotate(num_posts=Count('jobpost')).filter(is_active=True).order_by('-num_posts')
    ...
    template = 'mobile/jobs/companies_list.html' if request.is_mobile else 'jobs/companies_list.html'
    return render(request, template, data)
...
def skill_fresher_jobs(request, skill_name, **kwargs):
    ...
    if final_locations:
        return redirect(reverse('location_fresher_jobs', kwargs={'city_name': skill_name}), permanent=True)
    if request.POST.get('refine_search') == 'True':
        jobs_list, searched_skills, searched_locations, searched_industry, searched_edu, searched_states = refined_search(request.POST)
    elif final_skill:
        search_dict = QueryDict('', mutable=True)
    ...
    if jobs_list:
        ...
        field = get_social_referer(request)
        show_pop = True if field == 'fb' or field == 'tw' or field == 'ln' else False
        meta_title, meta_description, h1_tag = get_meta_data('skill_fresher_jobs', {'skills': searched_skills, 'fresher': True,
                                                                                    'final_skill': final_skill, 'page': page})
        data = {'job_list': jobs_list,
                'aft_page': aft_page,
                'after_page': after_page,
                'prev_page': prev_page,
        ...
        if searched_skills:
            ...
            status = 404
            skills = list(filter(None, request.POST.get('q', '').split(', '))) or [skill_name]
            reason = "Only valid Skill/city names are accepted"
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': 'Unfortunately, we are unable to locate the jobs you are looking for',
                                          'searched_job_type': 'Fresher', 'job_search': True,
                                          'reason': reason, 'searched_skills': skills, 'meta_title': meta_title,
                                          'meta_description': meta_description,
                                          'data_empty': status != 200}, status=status)
...
def location_fresher_jobs(request, city_name, **kwargs):
    ...
    if request.POST.get('refine_search') == 'True':
        jobs_list, searched_skills, searched_locations, searched_industry, searched_edu, searched_states = refined_search(request.POST)
        final_locations = final_locations + list(searched_states.values_list('name', flat=True))
    elif state:
    ...
    if jobs_list:
        ...
        field = get_social_referer(request)
        show_pop = True if field == 'fb' or field == 'tw' or field == 'ln' else False
        meta_title, meta_description, h1_tag = get_meta_data('location_fresher_jobs', {'locations': searched_locations,
        ...
        if final_locations:
            ...
            meta_title, meta_description = get_404_meta('location_404', {'city': location, 'fresher': True})
        else:
            status = 404
            meta_title = meta_description = ''
            location = list(filter(None, request.POST.get('location', '').split(', '))) or [city_name]
            reason = "Only valid Skill/city names are accepted"
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': 'Unfortunately, we are unable to locate the jobs you are looking for',
                                          'searched_job_type': 'Fresher', 'job_search': True,
                                          'reason': reason, 'meta_title': meta_title,
                                          'meta_description': meta_description,
                                          'searched_locations': location, 'data_empty': status != 200}, status=status)
...
def skill_location_walkin_jobs(request, skill_name, **kwargs):
    ...
    if jobs_list:
        ...
        show_pop = True if field == 'fb' or field == 'tw' or field == 'ln' else False
        if final_locations:
            meta_title, meta_description, h1_tag = get_meta_data('location_walkin_jobs', {'locations': searched_locations,
                                                                                          'walkin': True,
                                                                                          'final_location': set(final_locations),
                                                                                          'page': page,
                                                                                          'state': bool(state)})
        else:
            meta_title, meta_description, h1_tag = get_meta_data('skill_walkin_jobs', {'skills': searched_skills, 'walkin': True,
                                                                                       'final_skill': final_skill, 'page': page})
        data = {'job_list': jobs_list,
                'aft_page': aft_page,
                'after_page': after_page,
        ...
        if '-in-' in request.path:
            if final_locations:
                ...
                status = 200
                meta_title, meta_description = get_404_meta('location_404', {'city': location, 'walkin': True})
            else:
                location, skills = list(filter(None, request.POST.get('location', '').split(', '))) or [skill_name], []
                status = 404
                meta_title = meta_description = ''
        else:
            if final_skill:
                ...
                status = 200
                meta_title, meta_description = get_404_meta('skill_404', {'skill': skills, 'walkin': True})
            else:
                status = 404
                skills, location = list(filter(None, request.POST.get('q', '').split(', '))) or [skill_name], []
                meta_title = meta_description = ''
        reason = "Only valid Skill/City names are accepted in search field"
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': 'Unfortunately, we are unable to locate the jobs you are looking for',
                                          'searched_job_type': 'walk-in', 'job_search': True,
                                          'reason': reason, 'searched_skills': skills, 'meta_title': meta_title,
                                          'meta_description': meta_description,
                                          'searched_locations': location, 'data_empty': status != 200}, status=status)
...
def skill_location_wise_fresher_jobs(request, skill_name, city_name, **kwargs):
    current_url = reverse('skill_location_wise_fresher_jobs', kwargs={'skill_name': skill_name, 'city_name': city_name})
    if kwargs.get('page_num') == '1' or request.GET.get('page') == '1':
        return redirect(current_url, permanent=True)
    if 'page' in request.GET:
        url = current_url + request.GET.get('page') + '/'
    ...
    if jobs_list:
        ...
        field = get_social_referer(request)
        show_pop = True if field == 'fb' or field == 'tw' or field == 'ln' else False
        meta_title, meta_description, h1_tag = get_meta_data('skill_location_fresher_jobs', {'skills': searched_skills,
        ...
        reason = "Only valid Skill names are accepted in search field"
        skills = final_skill or list(filter(None, request.POST.get('q', '').split(', '))) or [skill_name]
        location = final_location or list(filter(None, request.POST.get('location', '').split(', '))) or [city_name]
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        if status == 200:
            meta_title, meta_description = get_404_meta('skill_location_404', {'skill': skills, 'city': location, 'fresher': True})
        else:
            meta_title = meta_description = ''
        return render(request, template, {'message': 'Unfortunately, we are unable to locate the jobs you are looking for',
                                          'searched_job_type': 'Fresher', 'job_search': True,
                                          'reason': reason, 'searched_skills': skills,
                                          'meta_title': meta_title, 'meta_description': meta_description,
                                          'searched_locations': location, 'data_empty': status != 200}, status=status)


def add_other_location_to_user(user, request):
    location = City.objects.filter(name__iexact=request.POST.get('other_location').strip())
    if location:
        user.current_city = location[0]
    else:
        location = City.objects.create(name=request.POST.get('other_location'), status='Disabled',
                                       slug=slugify(request.POST.get('other_location')), state=State.objects.get(id=16))
    ...
    if request.method == 'POST':
        ...
        if validate_user.is_valid():
            ...
            if not (User.objects.filter(email__iexact=request.POST.get('email')) or User.objects.filter(username__iexact=request.POST.get('email'))):
                ...
                user = User.objects.create(username=email, email=email, user_type='JS', registered_from=registered_from)
                ...
                if 'resume' in request.FILES:
                    ...
                    # conn.upload(
                    #     path, request.FILES['resume'], settings.AWS_STORAGE_BUCKET_NAME, public=True, expires='max')
                    # user.resume = path
                    ...
                    with open(file_path, 'wb+') as f:
                      for chunk in resume_file.chunks():
                         f.write(chunk)
                    # return file_path
                    # handle_uploaded_file(resume_file, filename)
                    user.resume = request.scheme + '://' + request.META['HTTP_HOST']+ "/"+file_path
                ...
                data = {'error': False, 'response': 'Registered Successfully', 'redirect_url': redirect_url}
                return HttpResponse(json.dumps(data))
            else:
                data = {
                    'error': True, 'response': 'User With This Email Already exists '}
                return HttpResponse(json.dumps(data))
        else:
        ...
        if validate_user.is_valid():
            ...
            if usr:
                ...
                if request.user.user_type == 'JS' and request.session.get('job_id'):
                    post = JobPost.objects.filter(id=request.session['job_id'], status='Live').first()
                    if post and usr.is_active and usr.profile_completion_percentage >= 50 or usr.resume:
                        job_apply(request, request.session['job_id'])
                        data['redirect_url'] = post.get_absolute_url() + '?job_apply=applied' if post else '/'
                    else:
                ...
                data = {
                    'error': True, 'response_message': "Username Password didn't match"}
            return HttpResponse(json.dumps(data))
        else:
            data = {'error': True, 'response': validate_user.errors}
            return HttpResponse(json.dumps(data))
...
def set_password(request, user_id, passwd):
    ...
    if request.method == 'POST':
        ...
        if validate_changepassword.is_valid():
            ...
            user.save()
            usr = authenticate(username=user.email, password=request.POST['new_password'])
            if usr:
                usr.last_login = datetime.now()
                usr.save()
                login(request, usr)
            ...
            return HttpResponse(json.dumps({'error': False, 'message': 'Password changed successfully', 'url': url}))
        else:
    ...
    template = 'mobile/404.html' if request.is_mobile else '404.html'
    return render(request, template, {'message': 'Not Found', 'reason': "URL may Expired"}, status=404)
...
def forgot_password(request):
    ...
    if form_valid.is_valid():
        ...
        if user:
            ...
            data = {'error': True, 'response_message': "User doesn't exist with this Email"}
        return HttpResponse(json.dumps(data))
...
def user_reg_success(request):
    if not request.user.is_authenticated:
        reason = "The URL may be misspelled or the page you're looking for is no longer available."
        template = 'mobile/404.html' if request.is_mobile else '404.html'
        return render(request, template, {'message': 'Sorry, Page Not Found',
                                          'reason': reason}, status=404)
    if request.method == 'POST':
        validate_user = UserEmailRegisterForm(request.POST, request.FILES, instance=request.user)
        if validate_user.is_valid():
    ...
    if request.user.registered_from == 'Social' and not request.user.mobile:
        template_name = 'mobile/profile/social_register.html' if request.is_mobile else 'candidate/social_register.html'
        return render(request, template_name)
    template = 'mobile/profile/user_reg_success.html' if request.is_mobile else 'candidate/user_reg_success.html'
...
def user_subscribe(request):
    ...
    if request.method == 'POST':
        ...
        if user and not user.user_type == 'JS':
            data = {'error': True,
                    'response_message': "Admin is not allowed to Subscribe" if user.is_staff else
                                        "Recruiter/Agency is not allowed to Subscribe"}
            return HttpResponse(json.dumps(data))
        if validate_subscribe.is_valid():
            all_subscribers = Subscriber.objects.filter(user=request.user) if request.user.is_authenticated \
                ...
                if not all_subscribers:
                    for skill in skills:
                        sub_code = subscribers_creation_with_skills(
                            email, skill, request.user if request.user.is_authenticated else '')
                    data = {'error': False, 'response': 'Successfully Subscribed'}
                else:
                    data = {'error': True, 'response_message': 'User with this email id already subscribed'}
            elif request.POST.getlist('skill'):
                all_subscribers = all_subscribers.filter(
                    skill__in=request.POST.getlist('skill'))
                if int(all_subscribers.count()) != int(len(request.POST.getlist('skill'))):
                    ...
                    data = {'error': True, 'response_message':
                            'User with this email id and skill(s) already subscribed'}
            else:
                data = {
                    'error': True, 'response_message': 'Please Enter atleast one skill'}
...
def process_email(request):
    ...
    if search:
        ...
        if not users:
            user = User.objects.create(username=email, email=email, user_type='JS', registered_from='Careers')
