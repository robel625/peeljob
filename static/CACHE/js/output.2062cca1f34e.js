...
$.ajax({type:'POST',dataType:'json',data:$('#ApplicantForm').serialize(),headers:{'X-CSRFToken':'{{ csrf_token }}'},url:url,headers:{'X-CSRFToken':'{{ csrf_token }}'},success:function(data){if(data.error==false){if(data.redirect_url){window.location=data.redirect_url;}
...
else{$('p.hint').remove();$('#userlogin_response_message').html('<p class="hint">'+data.response_message+'</p>');}}});});$('form#ApplicantFormRegister').submit(function(e){e.preventDefault();$('.login_form_button').prop('disabled',true);var formData=new FormData($(this)[0]);$.ajax({type:'POST',dataType:'json',data:formData,enctype:'multipart/form-data',headers:{'X-CSRFToken':'{{ csrf_token }}'},processData:false,contentType:false,url:'/registration/using_email/',headers:{'X-CSRFToken':'{{ csrf_token }}'},success:function(data){$('.login_form_button').prop('disabled',false);if(data.error==false){$('p.hint').remove();if(data.redirect_url){window.location=data.redirect_url;}
