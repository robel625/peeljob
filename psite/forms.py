...
class SimpleContactForm(forms.ModelForm):
    class Meta:
        model = simplecontact
        fields = ['first_name', 'last_name', 'comment', 'email', 'enquery_type', 'phone']
...
class UserEmailRegisterForm(forms.ModelForm):
    ...
    email = forms.EmailField(required=True, error_messages={'required': 'Email can not be blank', 'invalid': 'Please enter a valid email'})
    password = forms.CharField(required=True, error_messages={'required': 'Password can not be blank'})
    resume = forms.FileField(required=False)
    other_location = forms.CharField(required=False, error_messages={'required': "Other Location cannot be empty"})

    class Meta:
        model = User
        fields = ['email', 'password', 'mobile', 'technical_skills', 'current_city', 'year', 'month', 'resume']
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['technical_skills'].error_messages = {'required': 'Skills cannot be empty'}
    ...
    def clean_email(self):
        ...
        user = User.objects.filter(email__iexact=form_cleaned_data['email']).exclude(id=self.instance.id)
        if user.exists():
    ...
    def clean_password(self):
        ...
        if password and len(password) < 7:
            raise forms.ValidationError("The password must be at least %d characters long." % 7)
        return password
    ...
    def clean_mobile(self):
        ...
        if mobile:
            ...
            if not users:
                length = len(str(mobile)) < 9 or len(str(mobile)) > 13
    ...
    def clean_email(self):
        ...
        if user:
            if not user.is_jobseeker:
                raise forms.ValidationError("You have registered as " + user.get_user_type_display() + " and you can't login as Job Seeker")
            return username
        else:
