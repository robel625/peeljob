...
from peeldb.models import (Country, State, City, Skill, Language, Qualification, Industry, FunctionalArea, User, MailTemplate, Company,
                           JobPost, Question)
...
def validation_name(self, model):
    ...
    if model == "Country":
        if Country.objects.filter(name__iexact=form_cleaned_data['name']).exclude(id=self.instance.id):
            raise forms.ValidationError(model + " name Should be unique")

    if model == "State":
        if State.objects.filter(name__iexact=form_cleaned_data['name']).exclude(id=self.instance.id):
            raise forms.ValidationError(model + " name Should be unique")
    if model == "City":
        if City.objects.filter(name__iexact=form_cleaned_data['name']).exclude(id=self.instance.id):
            raise forms.ValidationError(model + " name Should be unique")

    if bool(re.search(r"[~\!@#\$%\^&\*\(\)_\+{}\":;'\[\]]", form_cleaned_data['name'])) or bool(re.search(r"[0-9]", form_cleaned_data['name'])):
...
class SkillForm(ModelForm):
    ...
    def clean_name(self):
        form_cleaned_data = self.cleaned_data
        if Skill.objects.filter(name__iexact=form_cleaned_data['name']).exclude(id=self.instance.id):
            raise forms.ValidationError("Skill name Should be unique")
        return form_cleaned_data['name']

    # def clean_slug(self):
    #     form_cleaned_data = self.cleaned_data
    #     if Skill.objects.filter(name__iexact=form_cleaned_data['slug']).exclude(id=self.instance.id):
...
class LanguageForm(ModelForm):
    ...
    def clean_name(self):
        ...
        if bool(re.search(r"[~\!@#\$%\^&\*\(\)_\+{}\":;,.'\[\]]", form_cleaned_data['name'])) or bool(re.search(r"[0-9]", form_cleaned_data['name'])):
            raise forms.ValidationError(
                "Language name Should not contain numbers")
        return form_cleaned_data['name']
...
class IndustryForm(ModelForm):
    ...
    def clean_name(self):
        ...
        if Industry.objects.filter(name=form_cleaned_data['name']).exclude(id=self.instance.id):
            raise forms.ValidationError("Industry name Should be unique")

        if bool(re.search(r"[0-9]", form_cleaned_data['name'])):
            raise forms.ValidationError(
                "Industry name Should not contain numbers")
        return form_cleaned_data['name']

    def clean_slug(self):
        form_cleaned_data = self.cleaned_data
        if Industry.objects.filter(slug=form_cleaned_data['slug']).exclude(id=self.instance.id):
            raise forms.ValidationError("Slug name Should be unique")
        return form_cleaned_data['slug']
...
class UserForm(ModelForm):
    ...
    class Meta:
        model = User
        fields = ['email', 'first_name', 'address',
                  'permanent_address', 'mobile', 'gender']
...
class MailTemplateForm(ModelForm):
    ...
    def clean_show_recruiter(self):
        if 'show_recruiter' in self.data.keys() and str(self.data['show_recruiter']) == 'True':
            if MailTemplate.objects.filter(applicant_status=self.data['applicant_status'], show_recruiter=True).exclude(id=self.instance.id):
                raise forms.ValidationError(
...
class CompanyForm(ModelForm):
    ...
    def clean_name(self):
        companies = Company.objects.filter(
            name=self.data['name']).exclude(id=self.instance.id)
        if companies:
            raise forms.ValidationError(
    ...
    def clean_website(self):
        if self.cleaned_data['website']:
            if (
                    re.match(r'^http://', self.cleaned_data['website']) or
                    re.match(r'^https://', self.cleaned_data['website']) or
                    re.match(r'^www.', self.cleaned_data['website'])):
                companies = Company.objects.filter(
                    website=self.data['website']).exclude(id=self.instance.id)
                if companies:
                    raise forms.ValidationError(
...
class JobPostTitleForm(ModelForm):
    ...
    def clean_title(self):
        if bool(re.search(r"[~\.,!@#\$%\^&\*\(\)_\+{}\":;'\[\]\<\>\|\/]", self.cleaned_data['title'])):
            raise forms.ValidationError(
                "Title Should not contain special characters and numbers")
        if JobPost.objects.filter(title=self.cleaned_data['title']).exclude(id=self.instance.id):
            raise forms.ValidationError(
