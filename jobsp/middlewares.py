...
class LowerCased(MiddlewareMixin):
    ...
    def process_request(self, request):
        if not re.match('/social/user/update/$', request.path) and not re.match('/logout/$', request.path):
            if request.user.is_authenticated and request.user.user_type == 'JS' and request.user.registered_from == 'Social' and not request.user.mobile:
                return redirect('/social/user/update/', permanent=False)
        if request.path == '/recruiter/':
            return redirect('/post-job/', permanent=False)
        detail_page = re.match('^/(?P<job_title_slug>[a-z0-9-.,*?]+)-(?P<job_id>([0-9])+)/$', request.path)
